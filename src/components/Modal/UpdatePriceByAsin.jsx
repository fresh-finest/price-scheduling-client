...
const UpdatePrice = ({ show, onClose }) => {
  ...
  const schedulePriceUpdate = (
    ...
    if (delayStart > 0) {
      setTimeout(() => {
        updateProductPrice(sku, newPrice)
          .then(() => {
            console.log(
              `Price updated to ${newPrice} at ${new Date().toLocaleString()}`
            );
          })
          .catch((error) =>
    ...
    if (delayEnd > 0) {
      setTimeout(() => {
        updateProductPrice(sku, originalPrice)
          .then(() => {
            console.log(
              `Price reverted to original price ${originalPrice} at ${new Date().toLocaleString()}`
            );
          })
          .catch((error) =>
