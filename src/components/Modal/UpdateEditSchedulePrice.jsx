...
const UpdatePriceFromList = ({
  ...
  const handleTimeChange = (scheduleType, identifier, index, key, value) => {
    const newTime = moment(value).tz("America/New_York").toDate();

    if (scheduleType === "weekly") {
      setWeeklyTimeSlots((prevSlots) => {
        const newSlots = [...(prevSlots[identifier] || [])];
        newSlots[index][key] = newTime;
  ...
  const convertTimeToUtc = (time)=>{
    return moment(time).tz("America/New_York").utc().format("HH:mm");
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Ensure that start and end dates are converted to EDT
    const convertedStartDate = moment(startDate).tz("America/New_York").toDate();
    const convertedEndDate = indefiniteEndDate ? null : moment(endDate).tz("America/New_York").toDate();
      ...
      if (weekly) {
        ...
        await saveScheduleAndQueueJobs(
          ...
          imageURL,
          convertedStartDate,
          indefiniteEndDate ? null : convertedEndDate,
          weekly,
                                  ...
                                  <DatePicker
                                    ...
                                    onChange={(time) =>
                                      handleTimeChange(
                                        ...
                                        index,
                                        "startTime",
                                        moment(time).tz("America/New_York").toDate()
                                      )
                                    }
                                    showTimeSelect
                                    showTimeSelectOnly
