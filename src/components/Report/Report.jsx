...
const Report = () => {
  ...
  // const fetchSalesComparison = async (startDate, endDate) => {
  //   setLoading(true);
  //   try {
  //     const skuArray = products.listings.map((product) => product.sellerSku);

  //     // Debugging: Log API parameters before request
  ...
  //       params: {
  //         startDate:
  //           dateRange[0]?.format("YYYY-MM-DD") || getDefaultStartDate(),
  //         endDate: dateRange[1]?.format("YYYY-MM-DD") || getDefaultEndDate(),
  //         skus: skuArray.join(","),
  //       },
  //     });
  ...
  const handlePageChange = (pageNumber) => {
    ...
    // Fetch data for the selected page and persisted date range
  ...
  // Handle Date Range Change (Ant Design RangePicker)
  const onRangeChange = (dates) => {
    if (dates && dates[0] && dates[1]) {
      // Format dates using dayjs to preserve local timezone
      const startDate = dates[0].format("YYYY-MM-DD"); // Keeps local date
      const endDate = dates[1].format("YYYY-MM-DD");

      // Update the selected range in the state
      setSelectedRange({ startDate, endDate });

      // Pass directly without relying on state
